...
OUT_DIR    <- file.path(REPO_ROOT, "output_concentration")
...
C_feed_ref <- unname(C_feed_iAs_ug_kg)            # 103.3 ug/kg reference feed iAs
conc_pop <- data.frame(
  meat   = ss_pbpk$C_iAs_muscle_ug,
  liver  = ss_pbpk$C_iAs_liv_ug,
...
run_simulation <- function(population_type, n_trials) {
  ...
  for (trial in 1:n_trials) {
    ...
    # feeding_rate retained only for distributional reporting/validation; in the
    # concentration-based model the cattle feed intake is already embedded in the
    # per-animal steady-state tissue concentrations and is NOT used in EF.
    ...
    # Resample ONE virtual animal per trial -> preserves cross-tissue and
    # BW-FR_feed correlations from the population PBPK.
    animal_idx      <- sample.int(n_animals, 1L)
    ...
    for (tissue in tissues) {
      ...
      ratio_val <- ratio_pop[[tissue]][animal_idx]   # C_tissue,ref / C_feed,ref (dimensionless)
      ...
      rec_tf[rec_i]     <- ratio_val

      total_ef <- total_ef + ratio_val * bioavailability * tissue_age_sum
    }
...
# (2a) Resampled concentration ratio vs the population ratio distribution.
# Ratios are resampled per-animal directly from the PBPK population, so the
# 2-sample KS D vs the population distribution should be ~0.
val_tf <- do.call(rbind, lapply(tissues, function(tis) {
  samp <- diag_gp$tf[diag_gp$tissue == tis]      # resampled ratios
  popr <- ratio_pop[[tis]]                        # population ratios
  k    <- suppressWarnings(ks.test(samp[is.finite(samp)], popr[is.finite(popr)]))
  data.frame(variable = paste0("ratio_", tis), target = "population resample",
             D = unname(k$statistic), p_value = k$p.value, stringsAsFactors = FALSE)
}))
...
pC3 <- ggplot(endpoint_data, aes(Cancer_Type, ML_Value, fill = Cancer_Type)) +
  ...
  labs(title = "ML by cancer endpoint (General Public, cattle)",
...
pC4 <- ggplot(sensitivity_results,
  ...
  labs(title = "Sensitivity tornado",
       subtitle = "Spearman rank correlation with the unified ML",
...
p_ml <- ga_canvas(ga_ml_c) +
  ...
  annotate("text", x = 5, y = 3.3, label = "vs. Current tAs feed MLs",
...
p_ga_title <- ggplot() +
  ...
  annotate("text", x = 5, y = 2.0,
           label = "Health-protective feed MLs for inorganic arsenic in cattle",
